...
class CameraViewController : UIViewController {
   ...
   @IBOutlet weak var renderView: RenderView!
    ...
    func loadCamera() {
        ...
        let finalView = RenderView(frame: CGRect(x: 0, y: 0, width: screenRect.size.width, height: screenRect.size.height))
        // Standard GPUImage3 methods. GPUImage2 APIs still work, so we use the documentation to come up with the following
        do {
            operation = RGBAdjustment()
